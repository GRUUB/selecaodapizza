<?php
	...
	class Promocao {
		...
		// Adicionar evento
		public function Add($nome, $imagem, $status){
			...
			if(!empty($_FILES['imagem']['name'])){
				...
				//PASTA
				$folder	= '/home/pizzaexp/public_html/painel/uploads/promocoes';
				...
				if($error != 0){
					...
					if(move_uploaded_file($tmp, $folder.'/'.$novoNome)){
						// Prepara o cadastro
						$query = $pdo->prepare("INSERT INTO promocoes (nome, imagem, status) VALUES (:nome, :imagem, :status)");
		...
		public function Edit($nome, $imagem, $status, $id){
			...
			if(!empty($_FILES['imagem']['name'])){
				...
				if($error != 0){
					...
					if(move_uploaded_file($tmp, $folder.'/'.$novoNome)){
						// Prepara a query
						$query = $pdo->prepare("UPDATE promocoes SET nome = ?, imagem = ?, status = ? WHERE id = ?");
						// Passa os parâmetros para executar a query
						$parametros = array($nome, $novoNome, $status, $id);
				...
				// Prepara a query
				$query = $pdo->prepare("UPDATE promocoes SET nome = ?, status = ? WHERE id = ?");
				// Passa os parâmetros para executar a query
				$parametros = array($nome, $status, $id);
				if($query->execute($parametros)){
					$this->MsgOk = "<script type='text/javascript'>
										$(window).load(function() {
		...
		public function Remove($id){
			...
			// Prepara a query
			$query = $pdo->prepare("DELETE FROM promocoes WHERE id = ?");
